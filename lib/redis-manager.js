...
function getClient() {
    ...
    var uniqueRedis = uniqueRedises[id];
    if(!uniqueRedis) {
        ...
        uniqueRedis = uniqueRedises[id] = redis.createClient(port, host, options);
        uniqueRedis.__id__ = id;
        redisRefs[id] = 0;
        if(selectDB) uniqueRedis.select(selectDB);
        if(options.forceReconnectForReadonlyErrors) {
            uniqueRedis.prototype.original_send_command = uniqueRedis.prototype.send_command;
            uniqueRedis.prototype.send_command = function (command, args, callback) {
                uniqueRedis.prototype.original_send_command.call(uniqueRedis, command, args, function (err) {
                    if (err && typeof(err.message) === 'string' &&
                        err.message.indexOf('READONLY') === 0 &&
                        uniqueRedis.stream) {
                        uniqueRedis.stream.end();
                    }
    ...
    var outRedis = binders(uniqueRedis);
    outRedis.__id__ = id;
    outRedis.__client__ = uniqueRedis;
...
function getAllClients(wanted) {
    return Object.keys(uniqueRedises).filter(typeof wanted === 'function' ?
            wanted : function () { return true; }).map(function (key) {
        return uniqueRedises[key];
    });
}
...
/*
 * Force redis reconnect for any client wanted. Defaults to all clients.
 */
function forceReconnect(wanted) {
    getAllClients(wanted).forEach(function (client) {
        client.stream.end();
    });
}
